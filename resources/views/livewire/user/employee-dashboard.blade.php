<div class="max-w-5xl mx-auto space-y-6 p-6">
    ...
    {{-- Search Placeholder (Matches Personnel Style) --}}
        ...
        <table class="w-full text-left border-collapse table-fixed">
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                @forelse($history as $log)
                    {{-- ADDED: Contextual Color Border on Hover --}}
                    <tr class="group border-l-4 border-transparent hover:border-{{ $log->type->color() }}-500 hover:bg-neutral-50/50 dark:hover:bg-neutral-800/30 transition-all">
                        ...
                        <td class="px-6 py-4 text-right align-top">
                            {{-- Icon Style Action: Matches Personnel --}}
            ...
            <div class="bg-white dark:bg-neutral-900 rounded-[2.5rem] p-8 w-full max-w-md shadow-2xl border border-neutral-200 dark:border-neutral-800 transform transition-all">
                ...
                <form wire:submit="submitMovement" class="space-y-5">
                    <div>
                        ...
                        <select wire:model="type" class="w-full bg-neutral-50 dark:bg-neutral-800 border-none rounded-2xl p-4 text-sm dark:text-white focus:ring-2 focus:ring-neutral-200 dark:focus:ring-neutral-700 transition-all mt-1 outline-none">
                            <option value="">Select...</option>
                            @foreach($types as $t) <option value="{{ $t->value }}">{{ $t->label() }}</option> @endforeach
                        </select>
                        @error('type') <span class="text-red-500 text-[10px] font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-[10px] font-black text-neutral-400 uppercase tracking-widest px-1">From</label>
                            <input type="datetime-local" wire:model="started_at" class="w-full bg-neutral-50 dark:bg-neutral-800 border-none rounded-2xl p-4 text-xs dark:text-white focus:ring-2 focus:ring-neutral-200 dark:focus:ring-neutral-700 transition-all mt-1 outline-none">
                        </div>
                        <div>
                            <label class="text-[10px] font-black text-neutral-400 uppercase tracking-widest px-1">Until</label>
                            <input type="datetime-local" wire:model="ended_at" class="w-full bg-neutral-50 dark:bg-neutral-800 border-none rounded-2xl p-4 text-xs dark:text-white focus:ring-2 focus:ring-neutral-200 dark:focus:ring-neutral-700 transition-all mt-1 outline-none">
                        </div>
                    </div>

                    <div>
                        <label class="text-[10px] font-black text-neutral-400 uppercase tracking-widest px-1">Remark / Purpose</label>
                        <textarea wire:model="remark" placeholder="e.g. Project meeting at Site A..." class="w-full bg-neutral-50 dark:bg-neutral-800 border-none rounded-2xl p-4 text-sm dark:text-white h-28 focus:ring-2 focus:ring-neutral-200 dark:focus:ring-neutral-700 transition-all mt-1 resize-none outline-none"></textarea>
                    </div>
                    ...
                    <div class="flex gap-4 pt-4 border-t border-neutral-100 dark:border-neutral-800">
                        ...
                        <button type="submit" class="flex-1 bg-neutral-900 dark:bg-white dark:text-black text-white py-4 rounded-2xl font-black text-sm shadow-xl hover:scale-[1.02] active:scale-95 transition-all">SAVE LOG</button>
                    </div>
                </form>
            </div>
        </div>
