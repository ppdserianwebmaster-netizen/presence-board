            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-6 w-full">
                ...
                <div class="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                    ...
                    <button wire:click="exportExcel" 
                        class="flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:shadow-emerald-500/20 transition-all duration-300 hover:scale-105 active:scale-95 whitespace-nowrap">
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y-2 divide-neutral-50 dark:divide-neutral-800/50">
                            @forelse($movements as $m)
                                <tr wire:key="{{ $m->id }}" class="group hover:bg-gradient-to-r hover:from-{{ $m->type->color() }}-50/30 hover:via-transparent hover:to-{{ $m->type->color() }}-50/30 dark:hover:from-{{ $m->type->color() }}-900/10 dark:hover:via-transparent dark:hover:to-{{ $m->type->color() }}-900/10 transition-all duration-300">
                                    <td class="px-8 py-5">
                                        <div class="space-y-1">
                                            <div class="font-black {{ $m->user?->trashed() ? 'text-rose-500 dark:text-rose-400' : 'text-neutral-900 dark:text-white' }} group-hover:text-{{ $m->type->color() }}-600 dark:group-hover:text-{{ $m->type->color() }}-400 transition-colors duration-300">
                                                {{ $m->user?->name ?? 'Unknown User' }}
                                                @if($m->user?->trashed()) 
                                                    <span class="text-[9px] font-black uppercase tracking-wider ml-1">(Deleted)</span> 
                                                @endif
                                            </div>
                                            <div class="text-[9px] text-neutral-400 dark:text-neutral-500 font-black tracking-wider uppercase flex items-center gap-2">
                                                ...
                                                {{ $m->user?->employee_id ?? 'ID: ' . $m->user_id }}
                                            </div>
                                        </div>
                                    </td>

                                    <td class="px-8 py-5">
                                        <span class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider bg-gradient-to-r from-{{ $m->type->color() }}-100 to-{{ $m->type->color() }}-50 text-{{ $m->type->color() }}-700 dark:from-{{ $m->type->color() }}-900/40 dark:to-{{ $m->type->color() }}-900/20 dark:text-{{ $m->type->color() }}-400 border-2 border-{{ $m->type->color() }}-200 dark:border-{{ $m->type->color() }}-800 shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all duration-300">
                                            <div class="w-2 h-2 rounded-full bg-{{ $m->type->color() }}-500 shadow-sm"></div>
                                            {{ $m->type->label() }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-8 py-5">
                                        <div class="space-y-1">
                                            <div class="font-black text-neutral-900 dark:text-white flex items-center gap-2 group-hover:text-{{ $m->type->color() }}-600 dark:group-hover:text-{{ $m->type->color() }}-400 transition-colors duration-300">
                                                <span>{{ $m->started_at->format('d M, H:i') }}</span>
                                                <svg class="w-4 h-4 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                                                </svg>
                                                <span>{{ $m->ended_at?->format('d M, H:i') ?? 'Ongoing' }}</span>
                                            </div>
                                            <div class="text-[9px] text-neutral-400 dark:text-neutral-500 font-black tracking-wider uppercase flex items-center gap-2">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    ...
                                    <td class="px-8 py-5 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-2">
                                            <button title="Edit Log" wire:click="edit({{ $m->id }})" 
                                            ...
                                            <button title="Delete Log" wire:click="delete({{ $m->id }})" wire:confirm="Delete this record permanently?" 
                                ...
                                <tr>
                                    <td colspan="4" class="px-8 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-xs text-neutral-400 dark:text-neutral-600 mt-1">Records will appear here once logged</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="bg-white dark:bg-neutral-900 rounded-3xl p-8 w-full max-w-xl border-2 border-neutral-200 dark:border-neutral-800 shadow-2xl animate-scale-in">
                <div class="flex items-start justify-between mb-6">
                    <div>
                        <h2 class="text-2xl font-black mb-1 dark:text-white bg-gradient-to-r from-neutral-900 to-neutral-600 dark:from-white dark:to-neutral-300 bg-clip-text text-transparent">
                            {{ $form->movement ? 'Update' : 'Create' }} Log
                        </h2>
                ...
                <form wire:submit="save" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        @error('form.user_id') <span class="text-rose-500 text-[10px] font-bold flex items-center gap-1 mt-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="datetime-local" wire:model="form.started_at" 
                                class="w-full border-2 border-neutral-200 dark:border-neutral-700 p-3.5 rounded-xl dark:bg-neutral-800 dark:text-white text-sm outline-none focus:ring-4 focus:ring-neutral-100 dark:focus:ring-neutral-800/50 focus:border-neutral-400 dark:focus:border-neutral-600 transition-all duration-300 font-semibold shadow-sm">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="datetime-local" wire:model="form.ended_at" 
                                class="w-full border-2 border-neutral-200 dark:border-neutral-700 p-3.5 rounded-xl dark:bg-neutral-800 dark:text-white text-sm outline-none focus:ring-4 focus:ring-neutral-100 dark:focus:ring-neutral-800/50 focus:border-neutral-400 dark:focus:border-neutral-600 transition-all duration-300 font-semibold shadow-sm">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea wire:model="form.remark" rows="3" placeholder="Reason for manual entry..." 
