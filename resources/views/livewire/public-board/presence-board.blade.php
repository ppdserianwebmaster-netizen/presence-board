...
<div wire:poll.8s="rotatePage" class="h-screen w-screen bg-black flex flex-col overflow-hidden font-sans border-[10px] border-black">
    ...
    <header class="h-[14%] border-b-2 border-slate-900 flex items-center justify-between px-8 mb-4 bg-black">
        <div class="flex items-center gap-10">
            ...
            <div class="h-16 w-[2px] bg-slate-800"></div>
            
            <div>
                <h1 class="text-5xl font-black text-white tracking-tighter uppercase leading-none">Presence Board</h1>
                <p class="text-slate-500 font-mono text-[11px] tracking-[0.5em] uppercase mt-2">Centralized Personnel Status And Presence Tracking</p>
            </div>
        </div>

        {{-- Clock: Alpine.js for zero-latency updates --}}
    ...
    <main class="flex-1 px-4">
        <div class="grid grid-cols-4 grid-rows-2 h-full w-full gap-4">
            @forelse($this->users as $user)
                ...
                <div wire:key="user-{{ $user->id }}" class="bg-[#080808] border border-slate-900 flex flex-col relative overflow-hidden shadow-[0_0_15px_rgba(0,0,0,0.5)]">
                            ...
                            @if($movement?->ended_at)
                                <div class="text-right bg-slate-900/40 p-3 border-l-2 border-slate-800">
                                    <span class="block text-[9px] text-slate-500 font-black uppercase mb-1 leading-none tracking-[0.2em]">
                                        @if($movement->ended_at->isToday()) Expected Today
                                        @elseif($movement->ended_at->isTomorrow()) Expected Tomorrow
                                        @else Expected Return @endif
                                    </span>
            ...
            @empty
                <div class="col-span-4 row-span-2 flex items-center justify-center">
                    <p class="text-slate-800 text-5xl font-black uppercase tracking-[0.5em] animate-pulse">Syncing Personnel...</p>
                </div>
            @endforelse
        </div>
    </main>

    {{-- Footer: Status Counts & System Info --}}
    <footer class="h-[10%] border-t-2 border-slate-900 flex items-center justify-between px-10 bg-black mt-2">
        
        {{-- Status Counts Loop (No extra file needed) --}}
        <div class="flex gap-8">
            @foreach([
                ['label' => 'Present', 'count' => $this->presentCount, 'color' => '#10b981'], {{-- emerald-500 --}}
                ['label' => 'Away',    'count' => $this->awayCount,    'color' => '#f59e0b'], {{-- amber-500 --}}
            ] as $stat)
                <div class="flex flex-col">
                    <span class="text-slate-600 text-[9px] font-bold uppercase tracking-widest">
                        {{ $stat['label'] }}
                    </span>
                    <span class="text-2xl font-black leading-none" style="color: {{ $stat['color'] }}">
                        {{ $stat['count'] }}
                    </span>
                </div>
            @endforeach
        ...
        <div class="flex flex-col items-center">
            <span class="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mb-2 leading-none">
                Rotation Index
            </span>
            <div class="flex gap-2 items-center">
                @for($i = 1; $i <= $this->totalPages; $i++)
                    <div class="h-2 w-12 transition-all duration-700 
                        {{ $i == $page ? 'bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.6)]' : 'bg-slate-900' }}">
                    </div>
                @endfor
            </div>
        </div>
